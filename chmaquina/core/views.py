...
#######################################################################################
#aquí comienza por prioridad Expropiativo
class PrioridadExpView(CreateView):
    model = EjecArchCh
    fields = ['archivo', 'memoria','kernel']
    

    success_url= reverse_lazy('prioridadExp')
    template_name = "core/base.html"
    global cambioCurso
    global ActivarVentLeer
    global ActivarVentLeerPaP
    ...
    def abrirArch(self, ruta):
        ...
        return leerLimp2
    #se utiliza para evitar la inanición de los procesos (compensación por envejecimiento)
    def mejoraPrioridad(self, listaProcesos): 
        try:
            listaKeys = list(listaProcesos.keys())
            menorPrior = listaProcesos[listaKeys[0]] 
            numProc =0
            #print('esto es el diccionario',listaProcesos, '\n')  
            #print(listaProcesos[0]['tRafaga'],'listpro')
            ...
            for clave, valor in listaProcesos.items():   
                ...
                print(listaProcesos[clave]['tRafaga'], '-trafaga de cada proceso')
                print(listaProcesos[clave]['tLlegada'], '-tllegada de cada proceso')
                print(menorPrior['tRafaga'], 'mas corto')
                """
                if listaProcesos[clave]['prioridad'] < menorPrior['prioridad']: # compara los tiempos de rafaga 
                    menorPrior = listaProcesos[clave]
                else:
                    numProc = listaKeys[0]

            menorPrior['prioridad'] += 10
            listaProcesos.update(menorPrior)
        except:
            print('no hay procesos para mejorar prioridad')
...
class RRView(CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not(tup): # cuando la base de datos se encuentra vacia 
            ...
            nombre=nombreArchivos[proEjec-1] #nombre del archivo que se muestra en el front
