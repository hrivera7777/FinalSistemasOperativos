...
class PaP: # clase paso a paso
    ...
    def getRB(self): #retorna el registro base del programa que se está ejecutando 
        return self.rb[self.proEjec]
    ...
    def agregarInstrMemoria(self): # idProg la variable global es proEjec, este metodo puede ser llamado desde views
        ...
        try:
            instrucMemor = self.memoria[self.rb[self.proEjec]]
        
        except:
            instrucMemor = ""

        if self.leer[0].rstrip() == instrucMemor :
            pass
        else: 
            for i in range(len(self.leer)):
                if i==0 :
    ...
    def ejecutarProgPaP(self, posMemEjec): # posMemEjec se requerirá si se llega  a una instrucción vaya o vayasi para cambiar la ejecucion del programa
        ...
        if int(posMemEjec) == self.rb[self.proEjec]:
            ...
            print('se agregaron las etiquetas: ',  self.etiquetas)
            varEjer = posMemEjec 
        
        elif int(posMemEjec) > 0: #si se llega a cambiar el orden de ejecucion del programa
            varEjer = posMemEjec 
        else:
